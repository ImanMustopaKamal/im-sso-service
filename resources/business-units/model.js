const helpers = require("../../utils/helpers"),
  ...
  table = db_sso+".ms_business_unit",
  tbl_alias = "business_unit",
  ...
  GetAll: async (tenant_id, params) => {
    try {
      ...
      if (params.organization_id) {
        filter.push({
          name: tbl_alias + ".organization_id",
      ...
      let columns = [
        ...
        tbl_alias + ".business_unit_template_id",
      ...
      let join = [
        {
          ...
          constraint: [
            {
              source: "org.id",
              dest: tbl_alias + ".organization_id",
              op: "eq",
            },
            {
              type: "and",
  ...
  Insert: async (body, user_id) => {
    try {
      let data_check_duplicate = {
        "Data": [
          { name: "organization_id", value: body.organization_id, op: "eq" },
          { name: "business_unit_template_id", value: body.business_unit_template_id, op: "eq" },
      ...
      let do_insert = await global_model.Insert(table, values, "UNIT");
